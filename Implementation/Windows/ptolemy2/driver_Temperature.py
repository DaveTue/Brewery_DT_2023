...
def run_simulate(fmuFilename, 
    ...
    vrInputs = []
    vrInputs  = [vrs['sg'], vrs['T_amb'], vrs['T_0']]  
    vrOutputs = vrs['Temperature'] 
    ...
    result = np.array(rows, dtype=np.dtype([('time', np.float64), ('Temperature', np.float64)]))
    print(result)
    plot_result(result)
...
if __name__ == '__main__':
    ...
    fmuFilename = os.path.join(os.getcwd(), 'FMUs', 'Temperature.fmu')
    startTime = 0.0
    stopTime = 1000000
    stepSize = 10

    startVals = [getMonitor('sg'), getMonitor('T_amb'), getMonitor('T_0')]
