...
def run_simulate(fmuFilename, 
    ...
    vrInputs = []
    vrInputs  = [vrs['Temperature'], vrs['sg_0'], vrs['grams_yeast'], vrs['batch_volume'] ]
    vrOutputs = vrs['rate_c_sugar'] 
    ...
    result = np.array(rows, dtype=np.dtype([('time', np.float64), ('rate_c_sugar', np.float64)]))
    print(result)
    plot_result(result)
...
if __name__ == '__main__':
    ...
    fmuFilename = os.path.join(os.getcwd(), 'FMUs', 'Reaction_model.fmu')
    startTime = 0.0
    stopTime = 1000000
    stepSize = 10
    startVals = [299.15, 1.054, 11.5, 20]
